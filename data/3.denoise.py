'''
DENOISING DATA WITH FFT (Fast Fourier Transform)

While collecting the EEG there are many artefact 
frequencies that are introduced into the waveform 
which are not directly produced by the human brain 
activity and therefore, reduce the sharpness of the
data structure. FFT helps us to remove weed (noise) 
from the grass (original EEG waveform). 
These weak frequencies were picked up while collecting 
EEG due to Digital(CPU, GPU) or Physiological artefacts
(respiratory, pulse and blinking).

FFT approach※: FFT approximates an arbitrary periodic 
function (in our case EEG sample for time = 1 second) 
with the help of the sum of Sinusoidal and Cosine waves 
of increasing frequencies. One could also understand FFT 
as a transformation of a signal from the time domain to 
